WITH
    RECURSIVE dates_d(date) AS (
        ...
        FROM dates_d
        WHERE date >= '2015-01-01' AND date < date(CURRENT_DATE, '-1 days')
    ),
    dates_raw AS (
    ...
    first_commit AS (SELECT '2015-01-01' AS date),
    commits AS (
...
LEFT JOIN stars ON dates.date = stars.date
...
with
    hashi as (select *
              from raw_authors
              where email like '%hashicorp.com'
                 or email like '%hashi%')
select a.*
from raw_authors as a
inner join hashi using (name)
;
